...
/usr/bin/curl $BLACKLIST | /usr/bin/fgrep -v ${WHITELIST[@]/#/-e } | sed -e '/^#/d' -e 's/address=/server=/' > ./dnsmasq.blacklist.txt.new

find . -name dnsmasq.blacklist.txt.new \( -size +1M -and -exec cat {} \> ./dnsmasq.blacklist.txt \; -or -true \) -delete
